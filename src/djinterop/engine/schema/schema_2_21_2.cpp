...
{
void schema_2_21_2::verify_master_list(sqlite::database& db) const
{
    // Schema 2.21.2 adds the `Smartlist` table.
    {
        ...
        ++iter;
        validate(iter, end, "table", "Smartlist", "Smartlist");
        ++iter;
...
void schema_2_21_2::verify_smartlist(sqlite::database& db) const
{
    // Schema 2.21.0 adds the `Smartlist` table.
    {
        table_info cols{db, "Smartlist"};
        auto iter = cols.begin(), end = cols.end();
        validate(iter, end, "lastEditTime", "DATETIME", 0, "", 0);
        ++iter;
        validate(iter, end, "listUuid", "TEXT", 1, "", 1);
        ++iter;
        validate(iter, end, "nextListUuid", "TEXT", 0, "", 0);
        ++iter;
        validate(iter, end, "nextPlaylistPath", "TEXT", 0, "", 0);
        ++iter;
        validate(iter, end, "parentPlaylistPath", "TEXT", 0, "", 0);
        ++iter;
        validate(iter, end, "rules", "TEXT", 0, "", 0);
        ++iter;
    ...
    {
        index_list indices{db, "Smartlist"};
        auto iter = indices.begin(), end = indices.end();
        validate(iter, end, "sqlite_autoindex_Smartlist_1", 1, "pk", 0);
        ++iter;
        validate(iter, end, "sqlite_autoindex_Smartlist_2", 1, "u", 0);
        ++iter;
        validate(iter, end, "sqlite_autoindex_Smartlist_3", 1, "u", 0);
        ++iter;
        validate_no_more(iter, end);
    }
    {
        index_info ii{db, "sqlite_autoindex_Smartlist_1"};
        auto iter = ii.begin(), end = ii.end();
        validate(iter, end, 0, "listUuid");
        ++iter;
        validate_no_more(iter, end);
    }
    {
        index_info ii{db, "sqlite_autoindex_Smartlist_2"};
        auto iter = ii.begin(), end = ii.end();
        validate(iter, end, 0, "title");
        ++iter;
        validate(iter, end, 1, "parentPlaylistPath");
        ++iter;
        validate_no_more(iter, end);
    }
    {
        index_info ii{db, "sqlite_autoindex_Smartlist_3"};
        auto iter = ii.begin(), end = ii.end();
        validate(iter, end, 0, "parentPlaylistPath");
        ++iter;
        validate(iter, end, 1, "nextPlaylistPath");
        ++iter;
        validate(iter, end, 2, "nextListUuid");
        ++iter;
        validate_no_more(iter, end);
    }
}

void schema_2_21_2::verify(sqlite::database& db) const
{
...
void schema_2_21_2::create(sqlite::database& db)
{
    ...
    db << "CREATE TABLE Track (  id INTEGER PRIMARY KEY AUTOINCREMENT,  "
          "playOrder INTEGER,  length INTEGER,  bpm INTEGER,  year INTEGER,  "
          "path TEXT,  filename TEXT,  bitrate INTEGER,  bpmAnalyzed REAL,  "
          "albumArtId INTEGER,  fileBytes INTEGER,  title TEXT,  artist TEXT,  "
          "album TEXT,  genre TEXT,  comment TEXT,  label TEXT,  "
          "composer TEXT,  remixer TEXT,  key INTEGER,  rating INTEGER,  "
          "albumArt TEXT,  timeLastPlayed DATETIME,  isPlayed BOOLEAN,  "
          "fileType TEXT,  isAnalyzed BOOLEAN,  dateCreated DATETIME,  "
          "dateAdded DATETIME,  isAvailable BOOLEAN,  "
          ...
          "originTrackId INTEGER,  trackData BLOB,  overviewWaveFormData BLOB, "
          ...
          "CONSTRAINT C_path UNIQUE (path),  FOREIGN KEY (albumArtId) "
    ...
    db << "CREATE TABLE Pack ( id INTEGER PRIMARY KEY AUTOINCREMENT,  "
          "packId TEXT,  changeLogDatabaseUuid TEXT,  changeLogId INTEGER , "
    ...
    db << "CREATE TABLE PlaylistEntity (  id INTEGER PRIMARY KEY "
          ...
          "membershipReference INTEGER,  CONSTRAINT C_NAME_UNIQUE_FOR_LIST "
          "UNIQUE (listId, databaseUuid, trackId),  FOREIGN KEY (listId) "
    ...
    db << "CREATE TABLE Playlist ( id INTEGER PRIMARY KEY AUTOINCREMENT,  "
          ...
          "isExplicitlyExported BOOLEAN,  CONSTRAINT C_NAME_UNIQUE_FOR_PARENT "
          "UNIQUE (title, parentListId),  "
          "CONSTRAINT C_NEXT_LIST_ID_UNIQUE_FOR_PARENT UNIQUE "
    ...
    db << "CREATE TABLE Smartlist (  listUuid TEXT NOT NULL PRIMARY KEY,  "
          ...
          "CONSTRAINT C_NAME_UNIQUE_FOR_PARENT UNIQUE "
          "(title, parentPlaylistPath),  CONSTRAINT "
          "C_NEXT_LIST_UNIQUE_FOR_PARENT UNIQUE (parentPlaylistPath, "
          "nextPlaylistPath, nextListUuid) );";
    ...
    db << "CREATE TRIGGER trigger_after_insert_Track_check_id AFTER INSERT ON "
          "Track  WHEN NEW.id <= (SELECT seq FROM sqlite_sequence WHERE "
          "name = 'Track') BEGIN  SELECT RAISE(ABORT, 'Recycling deleted "
    ...
    db << "CREATE TRIGGER trigger_before_insert_List BEFORE INSERT ON Playlist "
          ...
          "-(1 + nextListId)  WHERE nextListId = NEW.nextListId  AND "
          "parentListId = NEW.parentListId; END;";
    db << "CREATE TRIGGER trigger_after_insert_List AFTER INSERT ON Playlist "
          "FOR EACH ROW BEGIN  UPDATE Playlist SET   nextListId = NEW.id  "
          "WHERE nextListId = -(1 + NEW.nextListId)  AND parentListId = "
    ...
    db << "CREATE TRIGGER trigger_after_update_isPersistChild AFTER UPDATE ON "
          ...
          "UPDATE Playlist SET   isPersisted = 0  WHERE id IN (SELECT "
          "childListId FROM PlaylistAllChildren WHERE id=new.id); END;";
    ...
    db << "CREATE VIEW PlaylistPath AS WITH RECURSIVE Heirarchy AS (  SELECT "
          ...
          "h.parent = c.id  ORDER BY depth DESC ), OrderedList AS (  SELECT "
          "id , nextListId, 1 AS position  FROM Playlist  WHERE nextListId = 0"
          "  UNION ALL  SELECT c.id , c.nextListId , l.position + 1  FROM "
          "Playlist c  INNER JOIN OrderedList l  ON c.nextListId = l.id ), "
          "NameConcat AS (  SELECT   child AS id,   GROUP_CONCAT(name ,';') "
          "|| ';' AS path  FROM  (   SELECT child, name   FROM Heirarchy   "
          "ORDER BY depth DESC  )  GROUP BY child ) SELECT  id,  path,  "
          "ROW_NUMBER() OVER  (   ORDER BY   (SELECT COUNT(*) FROM (SELECT * "
          "FROM Heirarchy WHERE child = id) ) DESC,   (SELECT position FROM "
          "OrderedList ol WHERE ol.id = c.id) ASC  ) AS position FROM Playlist "
          "c LEFT JOIN NameConcat g USING (id);";
    db << "CREATE VIEW PlaylistAllParent AS WITH FindAllParent AS (  SELECT "
          "id, parentListId FROM Playlist  UNION ALL  SELECT recursiveCTE.id, "
    ...
    db << "CREATE TRIGGER trigger_before_delete_PlaylistEntity BEFORE DELETE "
          ...
          "SET   nextEntityId = OLD.nextEntityId  WHERE nextEntityId = OLD.id  "
          "AND listId = OLD.listId; END;";
    ...
    db << "CREATE VIEW PerformanceData AS SELECT  id AS trackId,  isAnalyzed,  "
          ...
          "thirdPartySourceId,  activeOnLoadLoops FROM Track;";
    db << "CREATE TRIGGER trigger_instead_insert_PerformanceData INSTEAD OF "
          ...
          "thirdPartySourceId = NEW.thirdPartySourceId,   activeOnLoadLoops = "
    ...
    db << "CREATE TRIGGER trigger_instead_update_isAnalyzed_PerformanceData "
          ...
          "BEGIN  UPDATE Track SET   isAnalyzed = NEW.isAnalyzed  WHERE "
    ...
    db << "CREATE TRIGGER trigger_instead_update_beatData_PerformanceData "
          ...
          "BEGIN  UPDATE Track SET   beatData = NEW.beatData  WHERE Track.id = "
          "NEW.trackId; END;";
    ...
    db << "CREATE TRIGGER trigger_instead_delete_PerformanceData INSTEAD OF "
          ...
          "isAnalyzed = NULL,   trackData = NULL,   overviewWaveFormData = "
          "NULL,   beatData = NULL,   quickCues = NULL,   loops = NULL,   "
          "thirdPartySourceId = NULL  WHERE Track.id = OLD.trackId; END;";
    db << "CREATE TRIGGER "
          "trigger_instead_update_activeOnLoadLoops_PerformanceData INSTEAD OF "
          "UPDATE OF activeOnLoadLoops ON PerformanceData FOR EACH ROW BEGIN  "
          "UPDATE Track SET   activeOnLoadLoops = NEW.activeOnLoadLoops  WHERE "
          "Track.id = NEW.trackId; END;";
    ...
    db << "CREATE TRIGGER trigger_after_insert_Pack_timestamp AFTER INSERT ON "
          "Pack FOR EACH ROW WHEN NEW.lastPackTime IS NULL BEGIN  UPDATE Pack "
          "SET lastPackTime = strftime('%s') WHERE ROWID = NEW.ROWID; END;";
    ...
    db << "CREATE TRIGGER trigger_after_update_Track_timestamp  AFTER UPDATE "
          "OF length, bpm, year, filename, bitrate, bpmAnalyzed, albumArtId,  "
          "title, artist, album, genre, comment, label, composer, remixer, "
          "key, rating, albumArt,  fileType, isAnalyzed, isBeatgridLocked, "
          "trackData, overviewWaveformData, beatData, quickCues,  loops, "
          "explicitLyrics, activeOnLoadLoops  ON Track  FOR EACH ROW BEGIN  "
          "UPDATE Track SET lastEditTime = strftime('%s') WHERE "
          "ROWID=NEW.ROWID; END;";
